...
![](Assets/3_fsc_pro_poject_catalog.png)
...
![](Assets/4_crs_ataya.png)

1.  LANDFIRE data (BpS, EVT, sClass rasters) was downloaded as “Best Fit UTM (NAD83 Datum)” so the Projected Coordinate System of the LANDFIRE rasters is NAD83_UTM_Zone_17N (Geographic Coordinate System was GCS_North American_1983). Given the size and location of our study area, we’ll use the Virginia State Plane (South) as our map coordinate system. *Note: unit is feet.*
...
![](Assets/7_combine_noattributes.png)
...
3.  Output raster = combine_studyArea *Reminder: if you decided to create a buffer around your study area, use the buffered study area feature class as your feature mask.*

![](Assets/11_extract.png)



The resultant "combine_studyArea raster" will only contain pixels within the Ataya study region:

![](Assets/12_combine_studyArea.png)
...
![](Assets/14_add_acres_field.png)


![](Assets/15_fields_view.png)
...
1.  Check the “GROUPVEG” then the “EVT_PHYS” fields to add them to the Pivot Table. Make sure that GROUPVEG is in the bottom left pane (“Rows”), NOT in the bottom right pane (“Values”). If it’s not, drag the field “GROUPVEG” to the top in the “Rows” pane.

    ![](Assets/8.3%20Screenshot%201.png)

2.  Create a new column to the left of “Row Labels”. For any bold GROUPVEG value (e.g. Conifer, Hardwood) type “Past” in bold in the cells to the left. For any normal EVT_PHYS value (e.g. Conifer-Hardwood, Developed) type “Present” in the cells to the left.

    ![](Assets/8.3%20Screenshot%202.png)
...
6.  Switch the order of the fields by dragging “EVT_PHYS” to the top of the Rows pane. Which GROUPVEG type was most converted to “Developed”? Note that our leftmost column—where we labeled “Past” and “Present”—is now switched around. If we switch the GROUPVEG and EVT_PHYS order, make sure we change the “Past” and “Present” values to avoid confusion.

![Switching order of fields](Assets/8.3%20Screenshot%203.png)
...
::: callout-note
When you open up the BpS2EVT worksheet, BPS_NAME, EVT_NAME, and Acres were already selected and it appeared an analysis was completed. I suggest we create a fresh pivot table where fields are empty. It did not appear that “COUNT” was selected.
:::
...
2.  In the Pivot Table, check "BPS_Name", "EVT_Name", and "COUNT"
    ...
    ![](Assets/8.4%20Screenshot%201.png)
